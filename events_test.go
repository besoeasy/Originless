...
func TestSignedEventLifecycle(t *testing.T) {
	...
	raw, owner := makeSignedEvent(t, createdAt, expiresAt, "chat", map[string]any{
		"user":    "alice",
		"message": "Hello world!",
	}, []string{"room:lobby"}, "")
...
func TestEventQueryCursorAndFilters(t *testing.T) {
	...
	store := newEventStore()
	handler := &eventHandler{store: store}
	first, _ := makeSignedEvent(t, now.Unix()-20, now.Unix()+3600, "chat", map[string]any{"message": "first"}, []string{"room:lobby"}, "")
	second, _ := makeSignedEvent(t, now.Unix()-10, now.Unix()+3600, "chat", map[string]any{"message": "second"}, []string{"room:lobby"}, "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef")
...
func TestEventValidationRejectsBadSignatureAndTTL(t *testing.T) {
	now := time.Now().Truncate(time.Second)
	raw, _ := makeSignedEvent(t, now.Unix(), now.Unix()+3600, "chat", map[string]any{"message": "hello"}, []string{}, "")
	...
	tooLong, _ := makeSignedEvent(t, now.Unix(), now.Unix()+MaxEventTTL+1, "chat", map[string]any{"message": "hello"}, []string{}, "")
...
func TestEventStoreBroadcastsToMatchingSubscriber(t *testing.T) {
	...
	publishTestEvent(t, &eventHandler{store: store}, raw)
